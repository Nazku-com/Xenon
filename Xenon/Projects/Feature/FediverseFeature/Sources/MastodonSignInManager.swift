...
struct MastodonSignInManager {
    ...
    func signIn(into url: URL, appInfo: AppInfoType) async -> Result<OauthData, SignInError> {
        ...
        case .success(let entity):
            return await continueSignIn(into: url, entity: entity.data, appInfo: appInfo)
        case .failure(let failure):
            return .failure(.appRegisterFailed(failure))
        }
    }
    ...
    private func continueSignIn(into url: URL, entity: MastodonAppEntity, appInfo: AppInfoType) async -> Result<OauthData, SignInError> {
        ...
        if let urlWithToken = try? await webAuthenticationSession.authenticate(
            ...
            return .failure(.createTokenFailed(nil))
        }
    }
    ...
    private func createOauthData(urlWithToken: URL, url: URL, entity: MastodonAppEntity, appInfo: AppInfoType) async -> Result<OauthData, SignInError> {
        ...
        case .success(let token):
            let userInfo = await service.request(api: MastodonAPI.checkUserInfo(from: url, token: token.data), dtoType: MastodonResponseDTO.Account.self)
            switch userInfo {
            case .success(let success):
                return .success(.init(url: url, nodeType: nodeType, token: token.data, user: success.data))
            case .failure:
                return .success(.init(url: url, nodeType: nodeType, token: token.data, user: nil))
            }
            
        case .failure(let failure):
            return .failure(.createTokenFailed(failure))
        }
    }
